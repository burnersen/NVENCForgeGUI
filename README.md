...
- 📜 **The list follows the work — until you take over.** The file list scrolls itself to whichever file is being converted, so you can see where it has got to without going looking for it. The moment you scroll it yourself it holds still and keeps out of your way; twenty seconds after you stop, it picks the thread up again. Same for the log, on every page, and there is no tick box to remember either way. And the convert list no longer clears itself when a batch is done: every line keeps what it was worth — **4.16 GB → 1.43 GB** side by side, with the CQ that got it there. Drop new files in and the finished ones make way, or use Clear. Taking files apart and putting them back together still tidy up after themselves, because there a second Start would do the work twice.
...
- 🧩 **Joining is a queue, not a single file.** Drop the pieces of as many films as you like into the Join page — they sort themselves into one job per film, going by the name the Split gave them, and the jobs run one after the other. Groups that are still missing their sound stay in the list, greyed out with the reason, instead of holding the rest up. Files in different folders are never mixed, even when they are called the same.
- ☑️ **The track chooser** — when a file has several audio tracks or subtitles, you tick what you want instead of typing numbers. Nothing arrives ticked; **Select all** and **Select none** are one click away. And **Use for all files** answers the rest of the queue for you: it remembers your choice *by language and by kind* — German sound and an English subtitle stays exactly that in every following file, whatever number those tracks happen to have there. A file that has no track of the language you picked is still put in front of you, so nothing is quietly taken that you did not want.
- ✂️ **Cut the black bars off** — for a 21:9 film sitting inside a 16:9 frame. Before letting it loose on a batch, press **Check black bars**: it writes one picture per file showing the proposed cut drawn on the *full* frame, and converts nothing. That is the only honest way to check a cut, because the finished file can no longer show you what was taken off it. Off unless you tick it — and an unticked box now means it, even if `autoCrop=true` sits in the config file (NVENCForge 1.22.0+). and what it does for you depends on Auto CQ — with a fixed CQ files come out about 6 % smaller, with Auto CQ on they come out *larger* and better. The bubble over the box says why. Needs NVENCForge 1.21.0 or newer; with an older one the box stays out of reach.
- ⚙️ **Every setting explained.** The Settings page is built from `NVENCForge_Config.ini` itself: hover over anything and it tells you what it does, what is allowed, and what your file currently says. The subtitle cleaner's phrase list is editable there too, instead of in a text editor. With NVENCForge 1.22.0 or newer the page is always complete: settings added by a newer converter are filled into your config file automatically, so nothing stays hidden just because your file predates the feature.
- ✋ **Stop one, or stop all.** Each converter has its own ✕; what is already encoded is kept as a playable preview. A file that has not started yet can be taken out of a running batch with the ✕ on its line — and it really is taken out of the queue the converter works from, not just off the screen.
...
- 🪞 **What you see is what is set.** Every option on the Convert page IS the one in your `NVENCForge_Config.ini`: the window fills its boxes from that file when it starts, and a change you make goes straight back into it. No more "GPU / NVENC" on screen while the file quietly says `encoder=cpu`. The file keeps two safety copies — `.bak` from before your last change, and one taken at start-up that is never overwritten while the window is open.
- 🏷️ **Keep your settings under a name.** A set of options you use often can be saved as a profile and picked again with one click — on the Convert page, and for the watched folder as well. A profile carries **all** of it: the boxes on the page AND the complete `NVENCForge_Config.ini`, so switching profiles really switches the quality target, the caps and the encoder knobs too, not just the six dropdowns.
...
| 🧩 **Join** | Put picture, audio and subtitles back into whole files. Same two answers as Split — and a real queue: drop the parts of ten films in and they sort themselves into ten jobs by name, one result each. |
...
The tool modes (Split, Join, DaVinci) always run one at a time. They copy instead of encoding — the disk is the limit there, not the card — and Split asks about tracks, where two dialogs at once would be a nuisance. Joining does run a queue, but it is a queue of jobs on that one slot: the converter builds exactly one file per run, so a batch of films becomes a batch of runs, one after the other.
